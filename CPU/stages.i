...
inline int getBlockNumber(int address)
{
...
char directAccess(int reg_id, int address)
{
    ...
    address_cache = block_num % size_cache;
...
char associativityAccess(int reg_id, int address)
{
    ...
    for(int cacheLine = 0; cacheLine < size_cache; ++cacheLine)
    {
        if(cache_memory[cacheLine].status == 1 && cache_memory[cacheLine].tag == block_num)
        {
            cacheHit++;
            wasHit = 1;
    ...
    if(!wasHit)
    {
        ...
        for(int cacheLine = 0; cacheLine < size_cache; ++cacheLine)
        {
            if(cache_memory[cacheLine].status == 0)
            {
                cache_memory[cacheLine].status = 1;
                cache_memory[cacheLine].tag = block_num;
...
# 128 "stages.c"
        ...
        for(int i = 0; i < block_size; i++)
        {
            cache_memory[victimLine].content[i] = memory_list[(first_position)+i].content;
        }
...
char setAssociativityAccess(int reg_id, int address, int associativity)
{
    ...
    int wasHit = 0;
    int gotIn = 0;
    cacheset_address = block_num % size_cache;
    int first_position = block_num * block_size;

    for(int cacheLine = 0; cacheLine < associativity; ++cacheLine)
    {
        if(cacheset_memory[cacheset_address].lines[cacheLine].status == 1 && cacheset_memory[cacheset_address].lines[cacheLine].tag == block_num)
        {
            cacheHit++;
            wasHit = 1;
    ...
    if(!wasHit)
    {
        ...
        for(int cacheLine = 0; cacheLine < associativity; ++cacheLine)
        {
            if(cacheset_memory[cacheset_address].lines[cacheLine].status == 0)
            {
                cacheset_memory[cacheset_address].lines[cacheLine].status = 1;
                cacheset_memory[cacheset_address].lines[cacheLine].tag = block_num;
...
# 189 "stages.c"
        ...
        for(int i = 0; i < block_size; i++)
        {
            cacheset_memory[cacheset_address].lines[victimLine].content[i] = memory_list[(first_position)+i].content;
        }
    ...
    switch(op)
    {
        ...
        case 3:
            ...
            if(associativity == 0)
            {
                block_num = getBlockNumber(address);
                address_cache = block_num % size_cache;
                first_position = block_num * block_size;
