...
inline void freeMemory(int count, ...)
{
    va_list args;
...
# 32 "main.c"
    ...
    scanf("%d", &size_cache);
    cache_memory = (struct cacheLine*) malloc(size_cache * sizeof(struct cacheLine));


    block_size = 1000000/total_blocks;
    ...
    if(associativity >= 2)
    {
        cacheset_memory = (struct cacheSet*) malloc(size_cache * sizeof(struct cacheSet));

        for(int i = 0; i < size_cache; ++i)
        {
            cacheset_memory[i].lines = (struct cacheLine*) malloc(associativity * sizeof(struct cacheLine));
        }
    ...
    while(!end)
    {
        switch(stage)
        {
            case 'F':
                ...
                {
                    current_command = instruction_list[inst_pointer - 1];
                    inst_pointer++;
                }
                else
                {
